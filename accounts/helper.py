...
def user_exists(phone_or_email)-> bool:
    ...
    try:
        if "@" in phone_or_email:
            ...
            print("phone_or_email", phone_or_email)
            user_is = User.objects.filter(phone_number=phone_or_email).exists()
            if user_is:
                user = User.objects.filter(phone_number=phone_or_email,user_is_verified=True).first()
                if user is not None and user.user_is_verified:
                    return True
                else:
                    print("---55--")
                    user_list = User.objects.filter(phone_number=phone_or_email,user_is_verified=False).all()
                    for i in user_list:
...
def send_phone_verification_otp(user: User | None = None, phone_number: str | None = None) -> None:
    ...
    try:
        ...
        print("verification_otp_is", verification_otp_is)
